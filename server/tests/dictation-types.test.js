...
describe("GET /dictation-types", function () {
    ...
    test("returns 500 when retrieving dictation types fails", async function () {
        ...
        expect(response.body).toEqual({
            error: "Errore durante il recupero dei tipi di dettato"
        });
    });
});
...
describe("POST /dictation-types", function () {
    ...
    test.each([
        ...
        "returns 400 when the name is %s",
        async function (caseName, invalidName) {
            ...
            expect(response.body).toEqual({
                error: "Nome del tipo di dettato non valido"
            });
        }
    );
    ...
    test("returns 409 when the name already exists", async function () {
        ...
        expect(response.body).toEqual({
            error: "Esiste già un tipo di dettato con questo nome"
        });
    });
    ...
    test("returns 500 when creating a type fails", async function () {
        ...
        expect(response.status).toBe(500);
        expect(response.body).toEqual({
            error: "Errore durante il salvataggio del tipo di dettato"
        });
    });
});
...
describe("DELETE /dictation-types/:id", function () {
    ...
    test.each([
        ...
        async function (caseName, invalidId) {
            ...
            const response = await request(app)
                .delete(`/dictation-types/${invalidId}`);

            expect(response.status).toBe(400);
            expect(response.body).toEqual({
                error: "ID del tipo di dettato non valido"
            });
        }
    );
