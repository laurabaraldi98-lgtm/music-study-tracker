document.body.innerHTML = `
    ...
    <div id="calendar-modal" hidden>
        <button id="close-calendar-modal">×</button>
...
const calendarCollectionFilter = document.getElementById(
    "calendar-collection-filter"
);

const getDictationsFromServerMock = jest.fn();

const formatDictationFromDatabaseMock = jest.fn(dictation => {
    ...
    return {
        ...
        dictationTypeName:
            dictation.dictation_type_name ||
            legacyTypeNames[dictation.type] ||
            dictation.type,
...
function getDisplayedYearMonth() {
    const title = calendarContainer.querySelector(
        ".calendar-header h3"
    ).textContent;
...
function getDateForDay(day) {
    const { month, year } = getDisplayedYearMonth();
...
async function prepareDictationDay(dictations) {
    await showCalendar([]);

    const date = getDateForDay(10);

    const preparedDictations = dictations.map(dictation =>
        makeDictation({
            date,
            ...dictation
        })
    ...
    await rerenderCalendar(preparedDictations);

    return getDayElement(10);
}

async function navigateTo(targetYear, targetMonth) {
    await showCalendar([]);

    let current = getDisplayedYearMonth();
    ...
    while (currentIndex !== targetIndex) {
        ...
        current = getDisplayedYearMonth();
        currentIndex = current.year * 12 + current.month;
    }
}
...
test("displays weekdays and all days of the month", async () => {
    ...
    const weekdays = Array.from(
        calendarContainer.querySelectorAll(".calendar-weekday")
...
test("marks days that contain dictations", async () => {
    ...
    const day = getDayElement(10);

    expect(day.classList.contains("has-dictation")).toBe(true);
    expect(day.tabIndex).toBe(0);

    expect(
        getDayElement(11).classList.contains("has-dictation")
    ).toBe(false);
});
...
test("filters calendar dictations by collection", async () => {
    ...
    const examDate = getDateForDay(10);
    const lessonDate = getDateForDay(11);

    calendarCollectionFilter.value = "Esame";
    ...
    expect(
        getDayElement(10).classList.contains("has-dictation")
    ).toBe(true);
...
test("opens the modal and displays default and custom types", async () => {
    const day = await prepareDictationDay([
        {
            id: 1,
            name: "Ritmico prova",
            type: "rhythmic",
        ...
        },
        {
            ...
            type: "wow",
            dictation_type_id: 4,
            dictation_type_name: "Wow",
            collection: "Lezione",
            correct_categories: ["Prova"]
        }
    ]);

    day.click();

    expect(calendarModal.hidden).toBe(false);
    expect(calendarModalTitle.textContent).toContain("Dettati del 10/");

    const modalText = calendarModalDictations.textContent;
    ...
    const links = Array.from(
        calendarModalDictations.querySelectorAll("a")
    );
    ...
    expect(document.activeElement).toBe(closeCalendarModalButton);
});

test("updates an open calendar when dictations change", async () => {
    await showCalendar([]);

    const date = getDateForDay(12);
    ...
    expect(
        getDayElement(12).classList.contains("has-dictation")
    ).toBe(true);
});

test("does not reload a closed calendar when dictations change", async () => {
    expect(calendarSection.hidden).toBe(true);
...
test("returns focus to the day when the modal closes", async () => {
    ...
    expect(calendarModal.hidden).toBe(true);
...
test("traps keyboard focus inside the modal", async () => {
    ...
    const videoLink = calendarModalDictations.querySelector("a");
    ...
    expect(document.activeElement).toBe(videoLink);
...
test("handles a month whose first day is Sunday", async () => {
    await navigateTo(2026, 10);

    expect(
        calendarContainer.querySelectorAll(".calendar-day.empty")
    ).toHaveLength(6);

    expect(
        calendarContainer.querySelector(".calendar-header h3").textContent
    ).toBe("Novembre 2026");
});
...
test.each([
    ...
    async ({
        ...
        await navigateTo(startYear, startMonth);
        ...
        expect(getDisplayedYearMonth()).toEqual({
            year: expectedYear,
            month: expectedMonth
        });
    }
