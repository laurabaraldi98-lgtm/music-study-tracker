...
const manageDictationTypesButton = document.getElementById("manage-dictation-types-button");
const dictationTypeManager = document.getElementById("dictation-type-manager");
const newDictationTypeInput = document.getElementById("new-dictation-type");
const addDictationTypeButton = document.getElementById("add-dictation-type-button");
const dictationTypesList = document.getElementById("dictation-types-list");
...
function renderDictationTypeSelect() {
    ...
    const selectedTypeStillExists = dictationTypes.some(function (type) {
...
function renderDictationTypesList() {
    ...
    if (dictationTypes.length === 0) {
        dictationTypesList.textContent = "Non ci sono tipi di dettato.";
        return;
    }
    ...
    for (const type of dictationTypes) {
        ...
        removeButton.classList.add("remove-dictation-type-button");

        removeButton.addEventListener("click", async function () {
            const confirmed = confirm(`Vuoi davvero rimuovere il tipo "${type.name}"?`);
            ...
            dictationTypes = dictationTypes.filter(function (savedType) {
...
async function loadDictationTypes() {
    try {
        dictationTypes = await getDictationTypesFromServer();
...
manageDictationTypesButton.addEventListener("click", function () {
    dictationTypeManager.hidden = !dictationTypeManager.hidden;

    if (dictationTypeManager.hidden) {
        manageDictationTypesButton.textContent = "Gestisci tipi di dettato";
    } else {
        manageDictationTypesButton.textContent = "Nascondi gestione tipi di dettato";
        renderDictationTypesList();
    }
});

addDictationTypeButton.addEventListener("click", async function () {
    const typedName = newDictationTypeInput.value.trim();
    ...
    try {
        savedType = await saveDictationTypeToServer({ name: formattedName });
    } catch (error) {
        console.error(error);
        alert(error.message);
        return;
...
newDictationTypeInput.addEventListener("keydown", function (event) {
    if (event.key === "Enter") {
